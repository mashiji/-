...
if __name__=='__main__':
    print('Load Training File Star...')
    data=np.loadtxt(r'C:\Users\mashiji\Desktop\optdigits.tra',dtype=np.float,delimiter=',')
    x,y=np.split(data,(-1,),axis=1)
    images=x.reshape(-1,8,8)
    y=y.ravel().astype(np.int)
    
    print('Load Test Data Start...')
    data=data=np.loadtxt(r'C:\Users\mashiji\Desktop\optdigits.tes',dtype=np.float,delimiter=',')
